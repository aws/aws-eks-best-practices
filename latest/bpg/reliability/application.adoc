...
Follow the
link:eks/latest/userguide/metrics-server.html[EKS
documentation,type="documentation"] to install metrics-server in your EKS cluster.
...
link:eks/latest/userguide/vertical-pod-autoscaler.html[EKS
Documentation,type="documentation"] includes a walkthrough for setting up VPA.
...
link:eks/latest/userguide/managed-node-update-behavior.html[nodes
are drained with a fifteen-minute timeout,type="documentation"]. After fifteen minutes, if
...
unavailable, such as
link:AWSEC2/latest/UserGuide/monitoring-instances-status-check_sched.html[EC2
maintenance,type="documentation"] events and
link:AWSEC2/latest/UserGuide/spot-interruptions.html[EC2
Spot interruptions,type="documentation"]. It uses the Kubernetes API to cordon the node to
...
Container Insights,type="documentation"] can monitor your cluster and workload and provide
...
link:eks/latest/userguide/control-plane-logs.html[EKS
provide five types of control plane logs,type="documentation"]:
...
or
link:AmazonCloudWatch/latest/monitoring/deploy-container-insights-EKS.html[CloudWatch
Container Insights,type="documentation"] in your cluster.
