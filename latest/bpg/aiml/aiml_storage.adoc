...
* link:eks/latest/userguide/s3-csi.html[Mountpoint for Amazon S3,type="documentation"]
* link:eks/latest/userguide/fsx-csi.html[Amazon FSx for Lustre,type="documentation"]
* link:eks/latest/userguide/fsx-openzfs-csi.html[Amazon FSx for OpenZFS,type="documentation"]
* link:eks/latest/userguide/efs-csi.html[Amazon EFS,type="documentation"]
* link:eks/latest/userguide/ebs-csi.html[Amazon EBS,type="documentation"]
...
**Amazon FSx for Lustre**: In scenarios where you have **multiple EC2 GPU compute instance** environment with latency-sensitive and high-bandwidth throughput dynamic workloads, such as distributed training and model serving, and you require native Amazon S3 data repository integration, we recommend link:fsx/latest/LustreGuide/what-is.html[Amazon FSx for Lustre,type="documentation"]. FSx for Lustre provides a fully managed high performance parallel filesystem that is designed for compute-intensive workloads like high-performance computing (HPC), Machine Learning.

You can link:eks/latest/userguide/fsx-csi.html[Install the FSx for Lustre CSI driver,type="documentation"] to mount FSx filesystems on EKS as a Persistent Volume (PV), then deploy FSx for Lustre file system as a standalone high performance cache or as an S3-linked file system to act as a high performance cache for S3 data, providing fast I/O and high throughput for data access across your GPU compute instances. FSx for Lustre can be deployed with either Scratch-SSD or Persistent-SSD storage options:
 ...
 ** **Standalone file system striping**: By default, a 4-component Lustre striping configuration is created for you via the link:fsx/latest/LustreGuide/performance.html#striping-pfl[Progressive file layouts (PFL),type="documentation"] capability of FSx for Lustre. In most scenarios you don't need to update the default PFL Lustre stripe count/size. If you need to adjust the Lustre data striping, then you can manually adjust the Lustre striping by referring to link:fsx/latest/LustreGuide/performance.html#striping-data[striping parameters of a FSx for Lustre file system,type="documentation"].
...
**Mountpoint for Amazon S3 with CSI Driver:** You can mount an S3 bucket as a volume in your pods using link:eks/latest/userguide/s3-csi.html[Mountpoint for Amazon S3 CSI driver,type="documentation"]. This method allows for fine-grained access control over which Pods can access specific S3 buckets. Each pod has its own mountpoint instance and local cache (5-10GB), isolating model loading and read performance between pods. This setup supports pod-level authentication with IAM Roles for Service Accounts (IRSA) and independent model versioning for different models or customers. The trade-off is increased memory usage and API traffic, as each pod issues S3 API calls and maintains its own cache.
...
For more details, see the link:eks/latest/userguide/s3-csi.html[Mountpoint for Amazon S3 CSI Driver,type="documentation"] in the Amazon EKS official documentation. We recommend monitoring the performance metrics of link:AmazonS3/latest/userguide/cloudwatch-monitoring.html[Amazon S3 with Amazon CloudWatch metrics,type="documentation"] if bottlenecks occur and adjusting your configuration where required.

=== Amazon EFS for shared model caches

In scenarios where you have a **multiple EC2 GPU compute instance environment** and have dynamic workloads requiring shared model access across multiple nodes and Availability Zones (e.g., real-time online inference with Karpenter) with moderate performance and scalability needs, we recommend using an Amazon Elastic File System (EFS) file system as a Persistent Volume through the EFS CSI Driver. link:efs/latest/ug/whatisefs.html[Amazon EFS,type="documentation"] is a fully managed, highly available, and scalable cloud-based NFS file system that enables EC2 instances and containers with shared file storage,  with consistent performance, and where no upfront provisioning of storage is required. Use EFS as the model volume, and mount the volume as a shared filesystem through defining a Persistent Volume on the EKS cluster. Each Persistent Volume Claim (PVC) that is backed by an EFS file system is created as an link:efs/latest/ug/efs-access-points.html[EFS Access-point to the EFS file system,type="documentation"]. EFS allows multiple nodes and pods to access the same model files, eliminating the need to sync data to each node's filesystem. link:eks/latest/userguide/efs-csi.html[Install the EFS CSI driver,type="documentation"] to integrate EFS with EKS.
...
To view performance specifications, see link:efs/latest/ug/performance.html[Amazon EFS performance specifications,type="documentation"].
...
* link:fsx/latest/LustreGuide/monitoring-cloudwatch.html[Amazon FSx console and its performance metrics,type="documentation"] to view the performance metrics related to your FSx file system.
* link:efs/latest/ug/accessingmetrics.html[Access Amazon CloudWatch metrics for Amazon EFS,type="documentation"] to view the performance metrics related to your EFS file system.
* link:AmazonS3/latest/userguide/cloudwatch-monitoring.html[Monitoring Amazon S3 metrics with Amazon CloudWatch,type="documentation"] to view performance details related to your S3 bucket.
