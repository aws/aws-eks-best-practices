...
==== https://docs.aws.amazon.com/eks/latest/best-practices/aiml-performance.html#_reduce_container_startup_times_by_preloading_container_images_into_data_volumes[Pre-caching Container Images]
...
When working with very large images that cannot easily be minimized, we recommend using the open source Seekable OCI (SOCI) Snapshotter configured in Parallel Pull and Unpack mode. This solution allows you to use existing images with no need to rebuild or modify your build pipelines. This option is especially effective when deploying workloads with very large images to high performance EC2 compute instances with high-throughput networking and high performance storage configurations as is typical with scaled AI/ML workloads.

SOCI Parallel Pull and Unpack dynamically improves end-to-end image pull performance through configurable parallelization strategies. Reducing the overall time to pull and prepare your container images directly impacts how fast you can deploy new workloads and also effects efficient cluster scaling as new nodes join your clusters. It’s important to understand that there are two main phases of image pulls, fetching layers from the registry to the node, and then unpacking and preparing those layers to create new containers.

For layer fetch optimization, SOCI establishes multiple concurrent HTTP connections per layer, effectively multiplying download throughput beyond the single-connection limitation. By splitting large layers into chunks and downloading them simultaneously across multiple connections, SOCI Parallel Mode can saturate your available network bandwidth and dramatically reduce download times. This approach is particularly effective for large layers common in AI/ML workloads, where a single layer can be several gigabytes.

For layer unpacking optimization, SOCI Parallel Mode introduces parallel unpacking across multiple layers simultaneously. Instead of waiting for each layer to be fully unpacked before starting the next, SOCI leverages your available CPU cores to decompress and extract multiple layers concurrently. This parallel processing approach transforms the traditionally I/O-bound unpacking phase into a CPU-optimized operation that scales with the number of available cores. The unpacking parallelization is carefully orchestrated to maintain filesystem consistency while maximizing throughput.

SOCI parallel pull Mode employs a dual-threshold control system with configurable parameters for both download concurrency and unpacking parallelism. This granular control provides great flexibility for you to finely tune SOCI Parallel Mode's behavior to meet your performance requirements and environment conditions. It is important to understand these parameters so that you can tune your runtime to provide the best pull performance possible. For more information on the solution and tuning tradeoffs, refer to the https://github.com/awslabs/soci-snapshotter/blob/main/docs/parallel-mode.md/[feature documentation] in the https://github.com/awslabs/soci-snapshotter/[SOCI project repository] on GitHub. 

For a more hands-on example with Karpenter on EKS, please refer to the https://github.com/aws-samples/karpenter-blueprints/tree/main/blueprints/soci-snapshotter/[Karpenter Blueprint using SOCI snapshotter parallel pull/unpack mode.
