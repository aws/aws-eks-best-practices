...
To get a quick sense of whether you are fully utilizing a GPU, you can use `nvidia-smi`, as shown in the following example. You can interpret the output as follows:
...
* If you use a GPU instance in an EKS node then it will have the `nvidia-device-plugin-daemonset` pod in the `kube-system` namespace by default. The `-l 5` flag says to output the metrics every 5 seconds.

```bash
kubectl exec nvidia-device-plugin-daemonset-xxxxx -n kube-system -- nvidia-smi \
   --query-gpu=power.draw,power.limit,temperature.gpu,utilization.gpu,utilization.memory,[memory.free](http://memory.free/),memory.used \ 
   --format=csv -l 5
```
