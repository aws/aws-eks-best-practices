...
This pattern also fits a fine-tuning lifecycle: collect domain data on CPU nodes, fine-tune on GPU, then deploy the quantized model back to CPU for inference at substantially lower cost than an LLM endpoint. Research from LoRA Land (https://arxiv.org/abs/2405.00732[arXiv:2405.00732]) shows that fine-tuned 7B models outperform GPT-4 on the majority of domain-specific tasks tested, making the "fine-tune a small model and run it on CPU" path viable for many production use cases.
...
* *Fine-tune the model on your domain*: The most expensive option, but the most effective. Research from LoRA Land (https://arxiv.org/abs/2405.00732[arXiv:2405.00732]) found that fine-tuned 7B models outperform GPT-4 on the majority of domain-specific tasks tested.
