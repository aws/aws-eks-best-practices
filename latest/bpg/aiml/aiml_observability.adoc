...
=== GPU Metrics Explained
GPUs are composed of multiple Streaming Multiprocessors which are composed of multiple different types of cores. While GPU utilization indicates that a GPU is doing work, it does not indicate how well it is utilizing all of the underlying hardware. Therefore, GPU Metrics need to be examined hierarchically: the topmost level gives an indication of the usage of the GPU, while digging deeper provides insight into how well the resources of the GPU are being used. At the topmost level (metrics available from nvidia-smi) are metrics such as GPU Utilization, Memory Utilization, GPU Power and GPU Temperature. GPU Utilization reports the percentage of time the GPU was executing at least one instruction over the sample period. Therefore, while 100% GPU Utilization may indicate the GPU was very active, being fully utilized, it can also indicate the GPU was mostly idle 100% of the time, if only a single instruction was executing the entire time. 

[#target-high-gpu-power-usage]
=== Target high GPU power usage
Underutilized GPUs indicate that the allocated GPU resources are not being fully leveraged by the workloads, leading to wasted compute capacity. For AI/ML workloads on Amazon EKS, we recommend monitoring GPU power usage as a proxy for high GPU usage and optimize resource efficiency. While GPU Utilization may be a tempting metric to use for understanding ho well the hardware is being used, it can be deceiving based on whether the components of the GPU are being used. Meanwhile, power usage can act as a proxy for the hardware usage as hardware that is being fully used will consume more power than hardware that is not. Underutilized GPUs waste compute capacity and increase costs, while over-scheduling can lead to contention and performance degradation.

We recommend setting up https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/deploy-container-insights-EKS.html[Cloudwatch Container Insights on Amazon EKS] to identify specific pods, nodes, or workloads with low GPU power consumption https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Container-Insights-metrics-enhanced-EKS.html[metrics]. It is easily integrated with Amazon EKS, enabling you to monitor GPU power consumption and adjust pod scheduling or instance types if power consumption falls below target levels. Alternatively, if this does not meet your specific requirements (e.g., advanced visualization), consider using NVIDIA's DCGM-Exporter alongside Prometheus and Grafana for Kubernetes-native monitoring. Both approaches provide insights into GPU metrics, enabling you to adjust pod scheduling or instance types if power usage falls below target levels. Check https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Agent-NVIDIA-GPU.html[NVIDIA metrics] like `nvidia_smi_power_draw` (GPU power consumption) and `nvidia_smi_temperature_gpu` (GPU temperature) via DCGM-Exporter or CloudWatch. Look for trends, such as consistently low power usage during certain hours or for specific jobs.
...
https://docs.nvidia.com/datacenter/dcgm/latest/gpu-telemetry/dcgm-exporter.html[NVIDIA DCGM Exporter] (e.g., GPU Power usage, GPU temperature, SM (Streaming Multiprocessor) Activity, SM Occupancy, and XID Errors), monitoring and optimizing inference serving for distributed workload insights with their native metrics using frameworks like 
...
* **Applicaiton Logs** - Application logs may help identify and root cause bottlenecks and failures. By understanding at which point in the workload lifecycle certain metrics occur, root causing issues becomes much easier.
* **Instance Logs** - Logs like those found in the instance journal or dmesg enable correlating hardware issues with application logs and instance metrics. 
