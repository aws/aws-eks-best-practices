...
* *Upgrade add-ons to cross-compatible versions.* Before upgrading the control plane, update your Amazon EKS managed add-ons to versions that are compatible with both the current and target Kubernetes versions. This keeps the add-on compatibility insight clear for both upgrade and rollback.
...
Amazon EKS surfaces rollback readiness insights under the `ROLLBACK_READINESS` category in cluster insights. These checks are your primary tool for assessing rollback safety.
...
* *Review disruption budgets before initiating rollback.* Amazon EKS provides rollback readiness insights that warn about NodePool disruption budgets set to 0 (blocks rollback indefinitely) and restrictive PDBs (slows rollback). Address these before initiating.
...
* *DescribeUpdate operation.* Use `describe-update` to check the current status of the rollback operation (`InProgress`, `Successful`, `Failed`, `Cancelled`).
...
* *AWS CloudFormation* allows up to 36 hours per resource. If rollback exceeds this, AWS CloudFormation triggers a stack rollback (roll-forward) that might leave the cluster in an unexpected state.
