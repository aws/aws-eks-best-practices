...
Please follow the instructions mentioned in the link:eks/latest/userguide/cni-increase-ip-addresses.html[EKS user guide,type="documentation"] to enable Prefix Delegation mode for Windows nodes.
...
The maximum number of IP addresses you can assign to a network interface depends on the instance type and its size. Each prefix assigned to a network interface consumes an available slot. For example, a `c5.large` instance has a limit of `10` slots per network interface. The first slot on a network interface is always consumed by the interface's primary IP address, leaving you with 9 slots for prefixes and/or secondary IP addresses. If these  slots are assigned prefixes, the node can support (9 * 16) 144 IP address whereas if they're assigned secondary IP addresses it can only support 9 IP addresses. See the documentation on link:AWSEC2/latest/UserGuide/using-eni.html#AvailableIpPerENI[IP addresses per network interface per instance type,type="documentation"] and link:AWSEC2/latest/UserGuide/ec2-prefix-eni.html[assigning prefixes to network interfaces,type="documentation"] for further information.
...
Use prefix delegation if you are experiencing Pod density issues on the worker nodes. To avoid errors, we recommend examining the subnets for contiguous block of addresses for /28 prefix before migrating to prefix mode. Please refer "`link:vpc/latest/userguide/subnet-cidr-reservation.html[Use Subnet Reservations to Avoid Subnet Fragmentation (IPv4),type="documentation"]`" section for Subnet reservation details.
...
For more details about the bootstrap configuration parameters for Windows nodes, please visit the documentation link:eks/latest/userguide/eks-optimized-windows-ami.html#bootstrap-script-configuration-parameters[here,type="documentation"].

[#windows-prefix-avoid]
...
* Enable/Disable the Prefix Delegation feature for Windows
* Update the node group using the steps mentioned link:eks/latest/userguide/update-managed-node-group.html[here,type="documentation"]. This performs similar steps as above but are managed by EKS.
