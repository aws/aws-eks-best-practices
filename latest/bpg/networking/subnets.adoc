...
Amazon EKS requires that you specify at least two subnets in two different Availability Zones when you create a cluster. The subnets you specify are known as cluster subnets. Amazon EKS provisions two cross-account ENIs (x-ENIs) in different Availability Zones to enable communication with your worker nodes. When you create a cluster, Amazon EKS creates 2–4 x-ENIs in your specified subnets. Amazon EKS always deploys the x-ENIs and uses them for cluster administration traffic such as log delivery, exec, and proxy. For more information about VPC and subnet requirements, see https://docs.aws.amazon.com/eks/latest/userguide/network_reqs.html#network-requirements-subnets[VPC and subnet requirements] in the Amazon EKS User Guide.
