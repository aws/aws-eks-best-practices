...
We suggest that you plan your infrastructure by combining these Pods' capacities. For a list of the maximum number of Pods supported by each instance type, see https://github.com/aws/amazon-vpc-cni-k8s/blob/master/misc/eni-max-pods.txt[eni-max-pods.txt] on GitHub.
...
If you are using custom AMI, make sure to set iptables forward policy to ACCEPT under https://github.com/awslabs/amazon-eks-ami/blob/main/templates/al2023/runtime/rootfs/etc/systemd/system/kubelet.service[kubelet.service]. Many systems set the iptables forward policy to DROP.  You can build custom AMI using https://packer.io/intro/why.html[HashiCorp Packer] and a build specification with resources and configuration scripts from the https://github.com/awslabs/amazon-eks-ami[Amazon EKS AMI repository on AWS GitHub]. You can update the https://github.com/awslabs/amazon-eks-ami/blob/main/templates/al2023/runtime/rootfs/etc/systemd/system/kubelet.service[kubelet.service] and follow the instructions specified https://aws.amazon.com/premiumsupport/knowledge-center/eks-custom-linux-ami/[here] to create a custom AMI.
