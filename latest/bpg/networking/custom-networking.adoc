...
* With custom networking enabled, no IP addresses assigned to the primary network interface are assigned to Pods. Only IP addresses from secondary network interfaces are assigned to Pods. For Pods to commnicate with the EKS Control Plane ENIs, suitable networking configuration is required. Please see the https://docs.aws.amazon.com/eks/latest/best-practices/subnets.html[VPC and Subnet Considerations] documention for more information.
