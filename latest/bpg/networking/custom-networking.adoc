...
image::cn-image-3.png[Architecture diagram showing EKS clusters deployed in non-routable 100.64.0.0/16 secondary CIDR range, with a private NAT gateway and NAT gateway in the routable RFC1918 CIDR range, connected via transit gateway to enable communication between VPCs with overlapping non-routable CIDRs]
...
image::cn-image-2.png[Architecture diagram showing Kubernetes worker nodes deployed on subnets from a secondary VPC CIDR range such as 100.64.0.0/10 without custom networking, where both nodes and Pods reside on the secondary subnet while the EKS control plane remains on the original primary subnet]
