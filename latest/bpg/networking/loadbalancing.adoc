...
link:eks/latest/userguide/aws-load-balancer-controller.html[AWS
Load Balancer controller,type="documentation"] will create and manage AWS Elastic Load
...
link:eks/latest/userguide/network-load-balancing.html[Network
Load Balancer (NLB),type="documentation"] which load balances received traffic at Layer 4 of
the OSI model. While when a Kubernetes Ingress object is created, the
AWS Load Balancer Controller creates an
link:eks/latest/userguide/alb-ingress.html[Application
Load Balancer (ALB),type="documentation"] which load balances traffic at Layer 7 of the OSI
model.
...
If you are utilizing link:link:eks/latest/userguide/automode.html[EKS Auto Mode,type="documentation"] the the AWS Load Balancer Controller is provided for you automatically; no installation is necessary.
...
link:eks/latest/userguide/aws-load-balancer-controller.html[AWS
Load Balancer Controller,type="documentation"] bridges the gap here. When you use AWS Load
...
link:elasticloadbalancing/latest/APIReference/Welcome.html[ELB
API,type="documentation"] to configure and start registering the Pod IP as a target on the
ELB.
...
link:elasticloadbalancing/latest/application/target-group-register-targets.html[target
registration,type="documentation"]. For a brief time you could have a state mismatch between
...
link:elasticloadbalancing/latest/APIReference/Welcome.html[ELB
API,type="documentation"] to start deregistering the Pod IP from the ELB. However depending
...
ELB also starts a
link:elasticloadbalancing/latest/application/load-balancer-target-groups.html#deregistration-delay[Deregistration
delay,type="documentation"] which is 300 seconds by default. During the deregistration
