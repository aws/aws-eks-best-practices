...
link:codebuild/latest/userguide/welcome.html[CodeBuild,type="documentation"]
instead.
...
// [IMPORTANT]
====
// Disabling ServiceAccount mounting does not prevent a pod from having // network access to the Kubernetes API. To prevent a pod from having any // network access to the Kubernetes API, you will need to modify the // link:eks/latest/userguide/cluster-endpoint.html[EKS // cluster endpoint access,type="documentation"] and use a // xref:iam-network-policy[NetworkPolicy] to block pod access.
====
