...
Alternately, use the
link:eks/latest/userguide/eks-optimized-amis.html[EKS
optimized AMI,type="documentation"] for your Kubernetes worker nodes. The EKS optimized AMI
...
node groups,type="documentation"] uses the first approach and will display a message in the
...
link:systems-manager/latest/userguide/session-manager.html[SSM
Session Manager,type="documentation"] when you need to remote into a host. Unlike SSH keys
...
link:systems-manager/latest/userguide/session-manager-working-with-install-plugin.html[Session
Manager plugin,type="documentation"] installed, you can then run
...
[NOTE]
====
You may also want to consider adding permissions to link:systems-manager/latest/userguide/getting-started-create-iam-instance-profile.html#create-iam-instance-profile-ssn-logging[enable Session Manager logging,type="documentation"].
====
...
You can use
link:inspector/latest/user/what-is-inspector.html[Amazon
Inspector,type="documentation"] to check for unintended network access to your nodes and for
...
on several
link:systems-manager/latest/userguide/ami-preinstalled-agent.html[Amazon
Machine Images (AMIs),type="documentation"] including
link:eks/latest/userguide/eks-optimized-ami.html[EKS
optimized Amazon Linux AMIs,type="documentation"]. Regardless of SSM agent status, all of
...
link:inspector/latest/user/enable-disable-scanning-ec2.html[Scanning
Amazon EC2 instances,type="documentation"].
