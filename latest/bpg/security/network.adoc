...
link:AmazonCloudWatch/latest/logs/AnalyzingLogData.html[CloudWatch
Log insights,type="documentation"] to perform advanced queries. From the logs, you can view
...
link:AmazonCloudWatch/latest/monitoring/ContainerInsights.html[CloudWatch
Container Insights,type="documentation"] to provide insights on your usage related to Network
Policies.
...
link:vpc/latest/userguide/flow-logs.html[AWS VPC
Flow Logs,type="documentation"] captures metadata about the traffic flowing through a VPC,
...
EKS uses
link:vpc/latest/userguide/VPC_SecurityGroups.html[AWS
VPC Security Groups,type="documentation"] (SGs) to control the traffic between the Kubernetes
...
link:eks/latest/userguide/sec-group-reqs.html.
...
groups for pods,type="documentation"]. With security groups for pods, you can assign an
...
link:eks/latest/userguide/security-groups-for-pods.html#security-groups-pods-considerations[considerations,type="documentation"]
...
[IMPORTANT]
====
The link:vpc/latest/userguide/amazon-vpc-limits.html#vpc-limits-security-groups[limits for security groups,type="documentation"] still apply when using security groups for pods so use them judiciously.
====
...
[IMPORTANT]
====
Security groups for pods relies on a feature known as link:AmazonECS/latest/developerguide/container-instance-eni.html[ENI trunking,type="documentation"] which was created to increase the ENI density of an EC2 instance. When a pod is assigned to an SG, a VPC controller associates a branch ENI from the node group with the pod. If there aren't enough branch ENIs available in a node group at the time the pod is scheduled, the pod will stay in pending state. The number of branch ENIs an instance can support varies by instance type/family. See link:eks/latest/userguide/security-groups-for-pods.html#supported-instance-types for further details.
====
...
traffic control in Istio,type="documentation"].
...
link:AWSEC2/latest/UserGuide/data-protection.html#encryption-transit[Encryption
in transit,type="documentation"] for further details on encryption in transit as well as the
...
link:app-mesh/latest/userguide/mutual-tls.html[mTLS,type="documentation"]
...
link:app-mesh/latest/userguide/virtual-node-tls.html[TLS
encryption,type="documentation"] with a private certificate issued by
link:acm/latest/userguide/acm-overview.html[AWS
Certificate Manager,type="documentation"] (ACM) or a certificate stored on the local file
...
The
link:elasticloadbalancing/latest/application/introduction.html[AWS
Application Load Balancer,type="documentation"] (ALB) and
link:elasticloadbalancing/latest/network/introduction.html[Network
Load Balancer,type="documentation"] (NLB) both have support for transport encryption (SSL and
...
link:acm/latest/userguide/acm-overview.html[AWS
Certificate Manager (ACM),type="documentation"] certificates using the host field. Starting
...
link:acm-pca/latest/userguide/create-CA.html[ACM
Private CA tech docs,type="documentation"]. Once you have a Private CA, install cert-manager
...
link:eks/latest/userguide/iam-roles-for-service-accounts.html[Service
Roles for IAM Accounts&#44; or IRSA,type="documentation"] can also be used. Please see the
...
* link:acm-pca/latest/userguide/PcaKubernetes.html[Private
CA Kubernetes cert-manager plugin user guide,type="documentation"].
