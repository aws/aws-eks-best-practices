...
Auto Mode supports easy integration with ADOT for comprehensive observability:

* Simplified deployment of ADOT Collector as an add-on
* Standardized approach to collecting metrics, traces, and logs
* Integration with various AWS and partner monitoring solutions

To enable the ADOT add-on:
...
Auto Mode clusters can leverage Amazon CloudWatch Container Insights for detailed performance and health monitoring:

* Collects, aggregates, and summarizes metrics and logs from containerized applications
* Provides insights at the cluster, node, pod, and container levels
* Enables automated detection of issues and setting of alarms
...
[NOTE]
====
While Auto Mode provides these monitoring and logging capabilities, it's crucial to configure them according to your specific security and operational requirements.
====
