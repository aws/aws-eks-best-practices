...
[IMPORTANT]
====
A Network Policy may prove ineffective if an attacker has gained access to underlying host. If you suspect that has happened, you can use link:vpc/latest/userguide/VPC_SecurityGroups.html[AWS Security Groups,type="documentation"] to isolate a compromised host from other hosts. When changing a host's security group, be aware that it will impact all containers running on that host.
====
...
link:AWSEC2/latest/UserGuide/terminating-instances.html#Using_ChangingDisableAPITermination[termination
protection,type="documentation"] can prevent this from happening.
link:autoscaling/ec2/userguide/as-instance-termination.html#instance-protection[Instance
scale-in protection,type="documentation"] will protect the node from a scale-in event.
...
link:whitepapers/latest/aws-security-incident-response-guide/welcome.html[AWS
Security Incident Response,type="documentation"].
