...
Account and cluster boundaries are related but independent architecture
decisions. Before selecting an implementation, use
xref:account-cluster-architecture[Choose an AWS account and cluster architecture]
to compare the reference patterns and evaluate requirements across account
management, cluster operations, access, networking, isolation, resilience, and
cost.

The following sections explain how to implement a multi account strategy for
EKS workloads using either a centralized or decentralized EKS cluster approach.
...
https://opengitops.dev/[GitOps] repositories to
