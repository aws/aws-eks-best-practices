...
found here,
link:eks/latest/userguide/control-plane-logs.html#enabling-control-plane-log-export.
...
[WARNING]
====
The maximum size for a CloudWatch Logs entry is link:AmazonCloudWatch/latest/logs/cloudwatch_limits_cwl.html[256KB,type="documentation"] whereas the maximum Kubernetes API request size is 1.5MiB. Log entries greater than 256KB will either be truncated or only include the request metadata.
====
...
SageMaker
link:sagemaker/latest/dg/randomcutforest.html[Random
Cut Forest,type="documentation"] algorithm to anomalous behaviors that warrant further
investigation.
