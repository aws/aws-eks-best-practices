...
include nodeAffinify. A Deployment could then use nodeSelectorTerms to
match `+billing-team+`.
...
!!! note It is not possible to set a global limit for the whole cluster.
Limits apply to specific NodePools.
...
`+karpenter.sh/karpenter.sh/do-not-disrupt+` annotation to the pod, you
