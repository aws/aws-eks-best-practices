...
Windows Server Core is a minimal installation option that is available as part of the link:eks/latest/userguide/eks-optimized-windows-ami.html[EKS Optimized Windows AMI,type="documentation"]. Deploying Windows Server Core has a couple of benefits. First, it has a relatively small disk footprint, being 6GB on Server Core against 10GB on Windows Server with Desktop experience. Second, it has a smaller attack surface because of its smaller code base and available APIs.
...
If access to the Windows node is needed, use link:systems-manager/latest/userguide/session-manager.html[AWS System Manager Session Manager,type="documentation"] to establish a secure PowerShell session through the AWS Console and SSM agent. To see how to implement the solution watch https://www.youtube.com/watch?v=nt6NTWQ-h6o[Securely Access Windows Instances Using AWS Systems Manager Session Manager]
...
Read up on the link:AWSEC2/latest/WindowsGuide/ec2-security.html[Security best practices for Amazon EC2 Windows instances,type="documentation"] to implement security controls at every layer.
