...
The following `windowsOptions` are the only documented https://kubernetes.io/docs/reference/kubernetes-api/workload-resources/pod-v1/#security-context[Windows Security Context options] while the rest are general https://kubernetes.io/docs/tasks/configure-pod-container/security-context/[Security Context options]

For a list of security context attributes that are supported in Windows vs linux, please refer to the official documentation https://kubernetes.io/docs/concepts/windows/intro/[here].
