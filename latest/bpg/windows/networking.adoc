...
For instructions on installing Calico on Amazon EKS, see https://docs.tigera.io/calico/latest/getting-started/kubernetes/managed-public-cloud/eks[Installing Calico on Amazon EKS] on the Tigera website.

In addition, the advice provided in the https://docs.aws.amazon.com/eks/latest/best-practices/network-security.html[Amazon EKS Best Practices Guide for Security - Network Section] applies equally to EKS clusters with Windows worker nodes, however, some features like "Security Groups for Pods" are not supported by Windows at this time.
