...
[NOTE]
====
For more information on how API A&F works please see the following link:eks/latest/best-practices/scale-control-plane.html#_api_priority_and_fairness[best practices guide,type="documentation"]
====
...
For more information, visit link:eks/latest/best-practices/scale-control-plane.html#_api_priority_and_fairness[API Priority and Fairness settings,type="documentation"] in the EKS Best Practices Guide.
...
Metrics are best used to find the problem area we want to look at and narrow both the timeframe and the search parameters of the problem. Once we have this data we want to transition to logs for more detailed times and errors. To do this we will turn our logs into metrics using link:AmazonCloudWatch/latest/logs/AnalyzingLogData.html[CloudWatch Logs Insights,type="documentation"].
