...
* *Standard mode* – By default, all {EKS} clusters use Standard mode. The control plane automatically scales up and down based on your workload demands. Standard mode dynamically allocates sufficient control plane capacity and is the recommended option for most use cases.
...
With {EKS} Provisioned mode, you choose from a set of scaling tiers (XL, 2XL, 4XL, and 8XL). With each tier, you get high, predictable performance from the cluster's control plane. Provisioned mode is particularly valuable for the following use cases:
...
With Provisioned mode, you can pre-allocate control plane capacity ahead of time and benefit from an enhanced 99.99% Service Level Agreement (SLA) measured in 1-minute intervals. For more information about {EKS} Provisioned mode, including tier specifications and pricing, see https://docs.aws.amazon.com/eks/latest/userguide/provisioned-control-plane.html[{EKS} Provisioned Control Plane] in the _{EKS} User Guide_.
