...
We recommend you use node autoscaling that reduces toil and integrates deeply with Kubernetes. link:eks/latest/userguide/managed-node-groups.html[Managed node groups,type="documentation"] and https://karpenter.sh/[Karpenter] are recommended for large scale clusters.
...
Karpenter allows workloads to declare the type of compute resources it needs without first creating node groups or configuring label taints for specific nodes. See the link:eks/latest/best-practices/karpenter.html[Karpenter best practices,type="documentation"] for more information. Consider enabling link:eks/latest/best-practices/karpenter.html#_scheduling_pods[consolidation,type="documentation"] in your Karpenter provisioner to replace nodes that are under utilized.
...
It is recommended that you use the latest link:eks/latest/userguide/eks-optimized-ami.html[Amazon EKS optimized Amazon Linux 2,type="documentation"] or link:eks/latest/userguide/eks-optimized-ami-bottlerocket.html[Amazon EKS optimized Bottlerocket AMI,type="documentation"] for your node image. Karpenter will automatically use the https://karpenter.sh/docs/concepts/nodepools/#instance-types[latest available AMI] to provision new nodes in the cluster. Managed node groups will update the AMI during a link:eks/latest/userguide/update-managed-node-group.html[node group update,type="documentation"] but will not update the AMI ID at node provisioning time.

For Managed Node Groups you need to update the Auto Scaling Group (ASG) launch template with new AMI IDs when they are available for patch releases. AMI minor versions (e.g. 1.23.5 to 1.24.3) will be available in the EKS console and API as link:eks/latest/userguide/update-managed-node-group.html[upgrades for the node group,type="documentation"]. Patch release versions (e.g. 1.23.5 to 1.23.6) will not be presented as upgrades for the node groups. If you want to keep your node group up to date with AMI patch releases you need to create new launch template version and let the node group replace instances with the new AMI release.

You can find the latest available AMI from link:eks/latest/userguide/eks-optimized-ami.html[this page,type="documentation"] or use the AWS CLI.
...
EBS is one of the easiest ways for workloads to have persistent storage, but it also comes with scalability limitations. Each instance type has a maximum number of link:AWSEC2/latest/UserGuide/volume_limits.html[EBS volumes that can be attached,type="documentation"]. Workloads need to declare what instance types they should run on and limit the number of replicas on a single instance with Kubernetes taints.
...
Amazon recommends using immutable infrastructure that is built, tested, and promoted from an automated, declarative system, but if you have a requirement to patch systems quickly then you will need to patch systems in place and replace them as new AMIs are made available. Because of the large time differential between patching and replacing systems we recommend using link:systems-manager/latest/userguide/systems-manager-patch.html[AWS Systems Manager Patch Manager,type="documentation"] to automate patching nodes when required to do so.
