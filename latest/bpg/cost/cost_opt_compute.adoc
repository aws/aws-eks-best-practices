...
Utilize tools such as https://www.youtube.com/watch?v=DfmQWYiwFDk[Goldilocks], https://www.youtube.com/watch?v=uITOzpf82RY[KRR], and https://aws.amazon.com/blogs/containers/aws-and-kubecost-collaborate-to-deliver-cost-monitoring-for-eks-customers/[Kubecost] which estimate resource requests and limits for your containers. Depending on the nature of the applications, performance/cost requirements, and complexity you need to evaluate which metrics are best to scale on, at what point your application performance degrades (saturation point), and how to tweak request and limits accordingly. Please refer to link:eks/latest/best-practices/node_and_workload_efficiency.html[Application right sizing,type="documentation"] for further guidance on this topic.
...
Using node groups can help the underlying compute resources do the expected thing by default, e.g. spread nodes across AZs, but not all workloads have the same requirements or expectations and it's better to let applications declare their requirements explicitly. For more information about Cluster Autoscaler, please see the link:eks/latest/best-practices/cas.html[best practices section,type="documentation"].
...
AWS resource link:tag-editor/latest/userguide/tagging.html[tags,type="documentation"] are used to organize your resources, and to track your AWS costs on a detailed level. They do not directly correlate with Kubernetes labels for cost tracking. It's recommended to start with Kubernetes resource labeling and utilize tools like https://aws.amazon.com/blogs/containers/aws-and-kubecost-collaborate-to-deliver-cost-monitoring-for-eks-customers/[Kubecost] to get infrastructure cost reporting based on Kubernetes labels on pods, namespaces etc.

Worker nodes need to have tags to show billing information in AWS Cost Explorer. With Cluster Autoscaler, tag your worker nodes inside a managed node group using link:eks/latest/userguide/launch-templates.html[launch template,type="documentation"]. For self managed node groups, tag your instances using link:autoscaling/ec2/userguide/ec2-auto-scaling-tagging.html[EC2 auto scaling group,type="documentation"]. For instances provisioned by Karpenter, tag them using https://karpenter.sh/docs/concepts/nodeclasses/#spectags[spec.tags in the node template].
...
Shared clusters may also have cluster level resource constraints such as IP exhaustion, Kubernetes service limits, or API scaling requests. You should review the link:eks/latest/best-practices/scale-control-plane.html[scalability best practices guide,type="documentation"] to make sure your clusters avoid these limits.
...
Fargate can scale workloads as small as .25 vCPU with 0.5 GB memory and as large as 16 vCPU with 120 GB memory. There are limits on how many link:eks/latest/userguide/fargate-pod-configuration.html[pod size variations,type="documentation"] are available and you will need to understand how your workload best fits into a Fargate configuration. For example, if your workload requires 1 vCPU with 0.5 GB of memory the smallest Fargate pod will be 1 vCPU with 2 GB of memory.
