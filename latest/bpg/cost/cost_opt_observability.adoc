...
You can link:AmazonCloudWatch/latest/logs/Working-with-log-groups-and-streams.html#SettingLogRetention[configure your retention periods,type="documentation"] in the AWS CloudWatch console or link:cli/latest/reference/logs/put-retention-policy.html[AWS API,type="documentation"] with the duration from 1 day to 10 years based on each log group. Having a flexible retention period can save log storage costs, while also maintaining critical logs.
...
Below is a snippet from the default AWS Distro for Opentelemetry (ADOT) EKS Addon Collector link:eks/latest/userguide/deploy-deployment.html[configuration,type="documentation"].
...
The X-Ray sampling rate is conservative by default. Define sampling rules where you can control the amount of data that you gather. This will improve performance efficiency while reducing costs. By link:xray/latest/devguide/xray-console-sampling.html#xray-console-custom[decreasing the sampling rate,type="documentation"], you can collect traces from the request only what your workloads needs while maintaining a lower cost structure.
