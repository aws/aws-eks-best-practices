...
When you create an Amazon EKS cluster, the IAM entity user or role, such as a federated user that creates the cluster, is automatically granted `system:masters` permissions in the cluster's RBAC configuration. This access cannot be removed and is not managed through the `aws-auth` ConfigMap. Since this can be used as a backdoor into the cluster, it is a good idea to create the cluster with a dedicated IAM role and regularly audit who can assume this role. After additional users have been granted access to the cluster through the `aws-auth` ConfigMap, you can also delete the role and recreate it only in an emergency / break glass situation. This can be particularly useful in production clusters which do not usually have direct user access.
