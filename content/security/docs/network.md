...
!!! caution 
    Unless you are running 1 pod per instance or dedicating a set of instances to run a particular application, security groups are considered to be too coarse grained to control network traffic.  Consider using network policies which are Kubernetes-aware instead. 
