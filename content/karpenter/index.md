...
In general, you should allow Karpenter to decide where to run pods, based on the set of constraints set in provisioners and pod deployments. Karpenter was written to optimally binpack pods onto instances. *At this time*, if you have a requirement to schedule pods on the same or different nodes, we recommend using [topologySpreadConstraints](https://karpenter.sh/v0.6.1/tasks/scheduling/#topology-spread). Rather than using podAffinity or podAntiAffinity to schedule pods across different fault domains/hosts, use [nodeSelectors](https://karpenter.sh/v0.6.1/tasks/scheduling/#selecting-nodes) and [taints](https://karpenter.sh/v0.6.1/tasks/scheduling/#taints-and-tolerations)for colocation. See Karpenter GitHub issues for updates to [podAffinity](https://github.com/aws/karpenter/issues/985) and [podAntiAffinity](https://github.com/aws/karpenter/issues/942).
