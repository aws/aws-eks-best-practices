...
def choose_context():
    try:
        ...
        print('Could not find kubeconfig in the default location.')
        return None
    ...
    selected, first_index = pick(contexts,
                                 title="Pick the context to run this against (press SPACE to mark, ENTER to continue): ",
...
def patch_aws_node(Context: str, **kwargs):
    ...
    try:
        AppsV1.patch_namespaced_daemonset(name='aws-node', namespace='kube-system', body=patch)
    except ApiException:
...
@click.option('--cluster-name', help='The EKS cluster name', required=False)
@click.option('--role-name', help='The role name for the AWS CNI', required=False)
...
def main(account, cluster_name, role_name, region):
    ...
    if context == None:
        api_client = create_clientset(ApiEndpoint=cluster.endpoint, ClusterName=cluster_name,
                                         Certificate=cluster.data)
